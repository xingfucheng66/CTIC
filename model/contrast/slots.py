...
class LoRALayer(nn.Module):
    def __init__(self, input_dim, output_dim, rank=8, alpha=1.0):
        ...
        # LoRA的A和B矩阵
        self.A = nn.Parameter(torch.Tensor(output_dim, rank))
        self.B = nn.Parameter(torch.Tensor(rank, input_dim))
        # 参数初始化
...
class ScouterAttention(nn.Module):
    ...
    def forward(self, inputs_pe, inputs, weight=None, things=None):
        ...
        for _ in range(self.iters):
            ...
            attn = torch.sigmoid(dots) # [B, cpt_num, visual_tokens_num]
            ...
            # print(attn.shape, attn2.shape, inputs.shape)
            # assert False
        if self.vis:
