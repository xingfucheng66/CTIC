...
from loaders.Caltech import Caltech
...
def get_transform(args):
    if args.dataset == "MNIST":
        ...
        return {"train": transform, "val": transform}
    elif args.dataset == "CUB200" or args.dataset == "ImageNet" or args.dataset == "imagenet" or args.dataset == "caltech" or args.dataset == "Caltech":
...
def select_dataset(args, transform):
    if args.dataset == "MNIST":
        ...
        return dataset_train, dataset_val
    elif args.dataset == "caltech" or args.dataset == "Caltech":
        dataset_train = Caltech(args.dataset_dir, "train", transform=transform["train"])
        dataset_val = Caltech(args.dataset_dir, "val", transform=transform["val"])
        return dataset_train, dataset_val
    elif args.dataset == "matplot":
...
def load_all_imgs(args):
    ...
    if args.dataset == "MNIST":
        ...
        cat = ["apple", "aquarium_fish", "baby", "bear", "beaver", "bed", "bee", "beetle", "bicycle", "bottle", "bowl", "boy", "bridge", "bus", "butterfly", "camel", "can", "castle", "caterpillar", "cattle", "chair", "chimpanzee", "clock", "cloud", "cockroach", "couch", "crab", "crocodile", "cup", "dinosaur", "dolphin", "elephant", "flatfish", "forest", "fox", "girl", "hamster", "house", "kangaroo", "computer_keyboard", "lamp", "lawn_mower", "leopard", "lion", "lizard", "lobster", "man", "maple_tree", "motorcycle", "mountain", "mouse", "mushroom", "oak_tree", "orange", "orchid", "otter", "palm_tree", "pear", "pickup_truck", "pine_tree", "plain", "plate", "poppy", "porcupine", "possum", "rabbit", "raccoon", "ray", "road", "rocket", "rose", "sea", "seal", "shark", "shrew", "skunk", "skyscraper", "snail", "snake", "spider", "squirrel", "streetcar", "sunflower", "sweet_pepper", "table", "tank", "telephone", "television", "tiger", "tractor", "train", "trout", "tulip", "turtle", "wardrobe", "whale", "willow_tree", "wolf", "woman", "worm"]
        train_imgs = datasets.CIFAR100('./data/cifar100', train=True, download=True, transform=None).data
        train_labels = datasets.CIFAR100('./data/cifar100', train=True, download=True, transform=None).targets
        val_imgs = datasets.CIFAR100('./data/cifar100', train=False, download=True, transform=None).data
        val_labels = datasets.CIFAR100('./data/cifar100', train=False, download=True, transform=None).targets
        return train_imgs, train_labels, val_imgs, val_labels, cat
        ...
        train = Caltech(args.dataset_dir, "train", transform=None).train
        val = Caltech(args.dataset_dir, "train", transform=None).val
        cat = Caltech(args.dataset_dir, "train", transform=None).category
        train_imgs, train_labels = filter(train)
        val_imgs, val_labels = filter(val)
        return train_imgs, train_labels, val_imgs, val_labels, cat
    elif args.dataset == "CUB200":
